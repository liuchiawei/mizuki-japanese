...
import { createBookingEvent, checkSlotAvailability } from '@/lib/google-calendar';
import { bookingFormSchema, isBookingTimeValid, createBookingError } from '@/lib/booking';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { studentName, studentEmail, studentTimezone, startTimeISO, note } = parseResult.data;
    ...
    if (!isAvailable) {
      return NextResponse.json(
        {
          success: false,
          error: createBookingError('SLOT_TAKEN', '抱歉，此時段剛被預約，請選擇其他時間'),
        },
        { status: 409 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: createBookingError('INTERNAL_ERROR', '創建預約時發生錯誤，請稍後再試'),
      },
      { status: 500 }
    );
  }
